...
// managed resource's usage is updated if the managed resource is updated to
// reference a misconfigured ProviderConfig.
func (u *LegacyProviderConfigUsageTracker) Track(ctx context.Context, mg Managed) error {
	lmg, ok := mg.(LegacyManaged)
	if !ok {
		return errors.New("managed resource is not a legacy Managed")
	}

	pcu, ok := u.of.DeepCopyObject().(LegacyProviderConfigUsage)
	if !ok {
		return errors.New("managed resource is not a legacy Managed")
	}

	gvk := lmg.GetObjectKind().GroupVersionKind()

	ref := lmg.GetProviderConfigReference()
	if ref == nil {
		return missingRefError{errors.New(errMissingPCRef)}
	}

	pcu.SetName(string(lmg.GetUID()))
