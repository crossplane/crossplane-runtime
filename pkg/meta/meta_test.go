...
func TestExternalNameIsDeterministic(t *testing.T) {
	cases := map[string]struct {
		o    metav1.Object
		want bool
	}{
		"HasExternalNameIsDeterministicAnnotationSetTrue": {
			o: func() metav1.Object {
				p := &corev1.Pod{}
				p.SetAnnotations(map[string]string{
					AnnotationKeyExternalNameIsDeterministic: "true",
				})
				return p
			}(),
			want: true,
		},
		"NoExternalNameIsDeterministicAnnotation": {
			o:    &corev1.Pod{},
			want: false,
		},
		"HasExternalNameIsDeterministicAnnotation": {
			o: func() metav1.Object {
				p := &corev1.Pod{}
				p.SetAnnotations(map[string]string{
					AnnotationKeyExternalNameIsDeterministic: "",
				})
				return p
			}(),
			want: false,
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			got := ExternalNameIsDeterministic(tc.o)
			if diff := cmp.Diff(tc.want, got); diff != "" {
				t.Errorf("ExternalNameIsDeterministic(...): -want, +got:\n%s", diff)
			}
		})
	}
}
