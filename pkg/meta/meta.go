...
const (
	...
	// AnnotationKeyExternalNameIsDeterministic is the key in the annotations
	// map of a resource that indicates that the name given to the resource by
	// the external system is deterministic. If this value is not "true", the
	// provider will not re-queue the managed resource in scenarios where creation
	// is deemed incomplete. This behaviour is a safeguard to avoid a leaked resource
	// due to a non-deterministic name generated by the external system. Conversely,
	// if this value is "true", signifying that the managed resources is deterministically
	// named by the external system, then this safeguard is ignored as it is safe
	// to re-queue a deterministically named resource.
	AnnotationKeyExternalNameIsDeterministic = "crossplane.io/external-name-is-deterministic"
...
// ExternalNameIsDeterministic returns true if the object has the
// AnnotationKeyExternalNameIsDeterministic annotation set to `true`.
func ExternalNameIsDeterministic(o metav1.Object) bool {
	return o.GetAnnotations()[AnnotationKeyExternalNameIsDeterministic] == "true"
}
