			...
			args: args{
				m: &fake.Manager{
					Client: &test.MockClient{
						...
						MockStatusUpdate: test.MockSubResourceUpdateFn(func(_ context.Context, obj client.Object, _ ...client.SubResourceUpdateOption) error {
							want := newLegacyManaged(42)
							want.SetManagementPolicies(xpv2.ManagementPolicies{xpv1.ManagementActionObserve})
							want.SetConditions(xpv2.ReconcileForbidden().WithObservedGeneration(42).WithMessage("External resource differs from desired state, but will not update: mock diff"))
